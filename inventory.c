...
static void handle_mouse_click(const bool left) {
    ...
    if (item_use(pItem->item_DB_index))
        inventory_decrement_item(pItem);
}
...
void inventory_initialize(void) {
    ...
    for (int i = 0; i < INVENTORY_SIZE; ++i) {
        if (pSave_current)
            inventory.item[i] = pSave_current->pInventory[i];
        else
    ...
    for (int i = 0; i < HOTBAR_COUNT; ++i) {
        if (pSave_current) {
            inventory.pHotbar[i].index_in_inventory = pSave_current->pHotbar_linked_index[i];
...
static void render_item(const int y,
...
static void render_hotbar(void) {
    ...
    for (int i = 0; i < HOTBAR_COUNT; ++i) {
        ...
        const WORD border_background = is_selected ? BACKGROUND_T_WHITE : BACKGROUND_T_DARKGRAY;
        ...
        for (int ty = 1; ty < slot_height - 1; ++ty)
            for (int tx = 1; tx < slot_width - 1; ++tx) {
                ...
                if (inventory.pHotbar[i].pPlayer_Item) {
                    ...
                    const item_information_t * const pItem_info = database_find_item_by_index(item_index);
                    if (pItem_info)
                    ...
                    if (item_type == ITEM_TYPE_MATERIAL)
                        texture_character = map_get_block_texture(item_index, texture_x, texture_y);
                    else if (item_type == ITEM_TYPE_TOOL)
                        texture_character = tool_get_texture(item_index, texture_x, texture_y);
...
*/
const bool inventory_add_item(const int item_DB_index, int quantity) {
    const item_information_t * const pItem_info = database_find_item_by_index(item_DB_index);
...
static void inventory_remove_item(player_item_t * const pItem) {
...
void inventory_decrement_item(player_item_t * const pItem) {
    const item_information_t *pItem_information = database_find_item_by_index(pItem->item_DB_index);
