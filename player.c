...
int find_ground_pos(int x)
{
    ...
    {
        block_info_t block = get_block_info_at(x, y);
        // 잔디, 흙, 돌 등 플레이어가 서 있을 수 있는 블록을 '지상'으로 간주
        if (block.type == BLOCK_GRASS || block.type == BLOCK_DIRT || block.type == BLOCK_SNOW)
        {
            // 찾은 지면 블록 바로 위를 플레이어의 Y 좌표로 설정
            int ground_y = (y - 1 >= 0) ? (y - 1) : 0;
            return ground_y;
        }
    }
    return map.size.y / 2;
