...
void InputItemFromUser(ItemDB* db) {
    int index, maxStack, type, baseDurability;
    char name[32];
    ...
    if (AddItemToDB(db, index, name, maxStack, type, baseDurability) == 0) {
        printf("추가 완료!\n\n");
    }
    else {
...
void SaveItemDBToCSV(const ItemDB* db, const char* filename) {
    ...
    // 헤더
    fprintf(fp, "index,name,maxStack,type,baseDurability\n");
...
void LoadItemDBFromCSV(ItemDB* db, const char* filename) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(line, "%d,%31[^,],%d,%d,%d",
            &index, name, &maxStack, &type, &baseDurability) == 5) {
            AddItemToDB(db, index, name, maxStack, type, baseDurability);
            printf("\n");
        }
    }
