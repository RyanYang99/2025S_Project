...
void Mob_Spawn_Time()
{
	...
	if (totalElapsedTime == 100 && difficult < 10) // 난도 조정용 임시 생성
	{
		difficult++;
	}
...
void MobSpawn(int player_x, int player_y) //플레이어 주변 최소범위 스폰제한 예정
{
	...
	for (int check_x = player_x - 50; check_x <= player_x + 50; check_x++) //플레이어 주변 x값 탐색
	{
		for (int check_y = player_y - 50; check_y <= player_y + 50; check_y++) //플레이어 주변 y값 탐색
		{
			...
			{
				if (map.ppBlocks[check_y + 1][check_x] == BLOCK_GRASS) //풀 자체가 땅위에 생성 되므로 풀 위이기만 하면 생성 되도록 설정
				{
					...
					if (Spawn_r == 0)
					{
						...
						mobs[mob_count].HP = difficult * 5;
						mobs[mob_count].atk = difficult * 2; //이후 추가 (난이도에 따라 증가)
						mob_count++;
					}
				}
			}
